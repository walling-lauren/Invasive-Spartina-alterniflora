...
#'@param replaceNAsWithNone if yes, na entries in colnamesForGrouping will be replaced by "none"
#'@return sampleTab
#'@author Marc W. Schmid \email{contact@@mwschmid.ch}.
#'@export
f.read.sampleTable <- 
  function(designTable, colNamesForGrouping = c(), replaceNAsWithNone = FALSE) {
  ...
  if (length(colNamesForGrouping) > 0) {
    ...
    if (length(colNamesForGrouping) > 1) {
      ...
      if (replaceNAsWithNone) {
        for (curCol in colNamesForGrouping) {
          sampleTab[[curCol]][is.na(sampleTab[[curCol]])] <- "none"
        }
      }
    } else {
      sampleTab$group <- sampleTab[[colNamesForGrouping]]
      if (replaceNAsWithNone) {
        sampleTab[[colNamesForGrouping]][is.na(sampleTab[[colNamesForGrouping]])] <- "none"
      }
    }
  }
  return(sampleTab)
...
#'@author Marc W. Schmid \email{contact@@mwschmid.ch}.
#'@export
f.read.vcf.file <- function(infileName, sampleName) {
  myData <- read.vcfR(infileName)
  colnames(myData@gt)[2:ncol(myData@gt)] <- sampleName
  colnames(myData@gt)[1] <- "FORMAT"
  colnames(myData@fix) <- c("CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO")
  genoTypes <- extract.gt(myData, element = "GT", as.numeric = FALSE)
  snpCoverage <- extract.gt(myData, element = "DP", as.numeric = TRUE)
  genoTypes <- genoTypes[,-(53:350)]
  snpCoverage<- snpCoverage[,-(53:350)]
  # correct Fallopia
...
#'@author Marc W. Schmid \email{contact@@mwschmid.ch}.
#'@export
f.load.methylation.bed <- function(infileName, removeMulticontextPositions = FALSE, percentages = FALSE) {
  f.print.message("WARNING, multiple context positions are not removed anymore by default if at least 90% of the samples match to one context")
  require("data.table")
  if (grepl("\\.gz$", infileName)) {
  ...
  if (removeMulticontextPositions) {
    ...
    # keep the ones that have the same context in at least 80% of the samples, assign them to that context
    if (length(multipleContexts) > 0) {
      subDat <- subset(myData, paste0(chr, '_', pos) %in% multipleContexts)#[,c("chr", "pos", "context", "samples_called")]
      maxFraction <- unlist(lapply(split(subDat$samples_called, paste0(subDat$chr, '_', subDat$pos)), function(x) max(x/sum(x))))
      #multipleContextsToKeep <- names(maxFraction)[maxFraction>=0.5]
      multipleContextsToKeep <- names(maxFraction)[maxFraction>=0.9]
      subDat <- subset(myData, paste0(chr, '_', pos) %in% multipleContextsToKeep)#[,c("chr", "pos", "context", "samples_called")]
      f.print.message("removing", length(multipleContexts)-length(multipleContextsToKeep), "positions with multiple contexts for which none is dominant.")
      if (nrow(subDat) > 0) {
        
        f.print.message("trying to reduce", length(multipleContextsToKeep), "positions with multiple contexts.")
        then <- Sys.time()
        newDat <- aggregate(subDat[,4:ncol(subDat)], by = list(chr = subDat$chr, pos = subDat$pos), function(x) sum(x, na.rm = TRUE))
        summarizeBy <- paste0(subDat$chr, '_', subDat$pos)
        maxContext <- unlist(lapply(split(subDat[,c("context", "samples_called")], summarizeBy), function(x) x[which.max(x[,2]),1]))
        newDat$context <- maxContext[paste0(newDat$chr, '_', newDat$pos)]
        #newDat$context <- paste0("m", newDat$context)
        newDat <- newDat[,colnames(subDat)]
        now <- Sys.time()
        print(now-then)
        # merge back
        oldDat <- subset(myData, !(paste0(chr, '_', pos) %in% multipleContexts))
        myData <- rbind(oldDat, newDat)
        myData <- myData[order(myData$chr, myData$pos, decreasing = FALSE), ]
      } else {
        myData <- subset(myData, !(paste0(chr, '_', pos) %in% multipleContexts))
      }
    }
...
#'@param limitToSamples a vector with sample IDs that whould be used
#'@return table with the snp data
#'@author Marc W. Schmid \email{contact@@mwschmid.ch}.
#'@export
f.read.and.combine.snp.and.methylation.data <- function(filteredSnpFile, filteredMethFile, replaceNASNPsWithRefVar = FALSE, limitToSamples = c()) {
  ...
  meDat <- f.load.methylation.bed(filteredMethFile, removeMulticontextPositions = FALSE, percentages = TRUE)
  if (length(limitToSamples) > 0) {
    commonSamples <- intersect(limitToSamples, colnames(snpDat))
    commonSamples <- intersect(commonSamples, colnames(meDat))
    notPresent <- setdiff(limitToSamples, commonSamples)
    if (length(notPresent) > 0) {
      cat("WARNING, THE FOLLOWING SAMPLES WERE NOT FOUND:\n")
      cat(paste0(notPresent, collapse = '\n'), '\n')
    }
    snpDat <- snpDat[,commonSamples]
    meDat <- meDat[,c("chr", "pos", "context", commonSamples)]
  }
...
#'@param subsetData a vector with sample names
#'@author Marc W. Schmid \email{contact@@mwschmid.ch}.
#'@export
f.load.many.distance.files <- function(allDistanceFiles, imputeNA = TRUE, subsetData = c()) {
  require("ape")
  allDistances <- list()
  for (curFile in allDistanceFiles) {
    ...
    #print(mean(is.na(temp)))
    if (length(subsetData) > 0) {
      f.print.message("using only a subset")
      commonSamples <- intersect(subsetData, rownames(temp))
      temp <- temp[commonSamples, commonSamples]
    }
    #print(temp)
    if (imputeNA) {
...
f.long.to.wide <- function(tab, futureRows, futureCols, futureEntries) {
  #empty <- ifelse(is.numeric(tab[[futureEntries]]), NA, "")
  empty <- NA
